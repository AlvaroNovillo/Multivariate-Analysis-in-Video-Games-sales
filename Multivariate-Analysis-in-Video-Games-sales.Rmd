...
covariance
```
As seen in the results, the correlation between North America and Europe are more correlated due to these countries having similar western cultures and how their markets are targeted in a similar way, while Japan is more alienated and the taste of its citizens differs significantly from that of Western cultures. We can showcase it using a heatmap as following.

```{r, echo=FALSE}
library(reshape2)
data1 <- melt(covariance) #To organize and format the data to create a heatmap
...
ggplot(data1, aes(x = Var1, y = Var2, fill = value)) +
  ...
  labs(title = "",
       ...
       y = "")
```
Due to the nature of the variables they present a positive correlation. If we'd like to know how much intercorrelation is present in our data, there are many different ways of calculating it so we've computed the different correlation measures to get a hint.
...
From the previous plot we can see the variables are right skewed, and there seems to be some outliers in the scatter plots. For positive skewness we can apply a log transformation, which would help to normalize the distribution of the variables. Many statistical methods, including linear regression and analysis of variance, assume that the residuals are normally distributed. Normalizing our data would also be useful to help achieve zero mean and unit variance. We can visualize it through boxplots.

```{r, echo=FALSE}
par(mfrow = c(1, 3))
boxplot(data[,1:3])
boxplot(log(data[,1:3]))
boxplot(log(scale(data[,1:3])))
```
